...
const object = getObject('2021.12.001');
...
function CaseLinks() {
  return <div className="case-links">{cases.map(group => <a key={group.name} href={caseUrl(group.name)}><span className="case-index">{caseStories[group.name].number}</span><div><h3>{group.name}</h3><p>{caseStories[group.name].line}</p></div><span aria-hidden="true">↗</span></a>)}</div>;
}
export function Home() {
  return <main id="content" tabIndex="-1" className="explore-page home-page">
    <section className="home-hero"><div className="hero-copy"><p className="eyebrow">Fashion · Form · Antiquity</p><h1>Ancient World<br/><em>in Modern</em><br/>Threads</h1><p className="hero-intro">{site.introduction}</p><a className="primary-link" href="#/map">Explore the exhibition <span aria-hidden="true">↗</span></a><a className="quiet-link" href={objectUrl(object.id)}>Begin with a single garment →</a></div><figure className="hero-image"><img src={object.image} alt={object.alt}/><figcaption><span>Fluting / Mariano Fortuny</span><span>c. 1920</span></figcaption><span className="image-marker" aria-hidden="true">01</span></figure></section>
    ...
    <section className="theme-section"><div className="section-heading"><div><p className="eyebrow">Find a thread to follow</p><h2>Explore by theme</h2></div><a className="quiet-link" href="#/map">See the spatial map ↗</a></div><CaseLinks/></section>
    ...
    <div className="map-layout"><section className="map-panel" aria-label="Exhibition layout">
      <div className="floor-plan">
        ...
        {displays.map(([name, position]) => {
          ...
          const content = <><span>{caseStories[name].number}</span><strong>{name}</strong>{!muted && <small aria-hidden="true">↗</small>}</>;
...
export function CasePage({ group }) {
  ...
  return <><Breadcrumbs group={group.name}/><main id="content" className="explore-page case-page" tabIndex="-1"><header className="case-heading"><div><p className="eyebrow">Case {story.number} / {story.location}</p><h1>{group.name}</h1><p className="case-subtitle">{story.line}</p></div><p className="case-lead">{label?.lead || story.description}</p></header>
    ...
    {group.alternates?.length > 0 && <section className="alternate-section"><p className="eyebrow">Alternate / not in the display sequence</p><h2>From the study collection</h2>{group.alternates.map(id => <a className="quiet-link" key={id} href={objectUrl(id)}>{getObject(id).designer}: {getObject(id).title} →</a>)}</section>}
    <nav className="case-connections" aria-label="Continue through the exhibition"><a href="#/map">← Back to the map</a><a href={caseUrl(cases[(cases.indexOf(group)+1)%cases.length].name)}>Explore {cases[(cases.indexOf(group)+1)%cases.length].name} →</a></nav>
  </main></>;
}
